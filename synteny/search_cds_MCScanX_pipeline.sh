...
# usage: bash search_cds_MCScanX_pipeline.sh queryTag dbTag
# usage ex: bash search_cds_MCScanX_pipeline.sh Jcinerea_138 Jailantifolia_136
...
outputFolder=$outputDir"/synteny_cds_MCScanX"
...
bash makeDB_blast.sh $dbFile $outputFolder"/"dbTag"_db" "nucl"

# make the second blast able database
bash makeDB_blast.sh $queryFile $outputFolder"/"queryTag"_db" "nucl"
...
bash search_blast.sh $queryFile $outputFolder"/"dbTag"_db" $outputFolder $outputFolder"/"$dbTag"_"$queryTag".blast" 5 "tblastx"

# 17. For each pair of genomes, switch the query and target genomes for a second execution.
bash search_blast.sh $dbFile $outputFolder"/"$queryTag"_db" $outputFolder $outputFolder"/"$queryTag"_"$dbTag".blast" 5 "tblastx"
...
bash search_blast.sh $dbFile $outputFolder"/"$dbTag"_db" $outputFolder $outputFolder"/"$dbTag"_"$dbTag".blast" 6 "tblastx"

# perform second within genome all-against-all BLASTP search
bash search_blast.sh $queryFile $outputFolder"/"$queryTag"_db" $outputFolder $outputFolder"/"$queryTag"_"$queryTag".blast" 6 "tblastx"
